...
async function syncFollowers() {
  ...
  const redisSaved = await redisCmd('SET', 'phuocthanh_oa_followers', JSON.stringify(profiles));
  if (redisSaved !== null) {
    await redisCmd('SET', 'phuocthanh_oa_followers_synced_at', _syncedAt);
  } else {
    saveToFile(profiles);
  }
...
async function getStoredFollowers() {
  ...
  // Thử Redis
  const raw = await redisCmd('GET', 'phuocthanh_oa_followers');
  if (raw) {
    try {
...
async function getSyncedAt() {
  ...
  const fromRedis = await redisCmd('GET', 'phuocthanh_oa_followers_synced_at');
