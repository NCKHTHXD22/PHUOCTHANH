...
async function refreshAccessToken() {
  ...
  await redisCmd('SET', 'phuocthanh_zalo_access_token', access_token);
  if (refresh_token) await redisCmd('SET', 'phuocthanh_zalo_refresh_token', refresh_token);
...
(async () => {
  const savedAccess = await redisCmd('GET', 'phuocthanh_zalo_access_token');
  const savedRefresh = await redisCmd('GET', 'phuocthanh_zalo_refresh_token');
...
async function setTokensManually(accessToken, refreshToken) {
  ...
  await redisCmd('SET', 'phuocthanh_zalo_access_token', accessToken);
  await redisCmd('SET', 'phuocthanh_zalo_refresh_token', refreshToken);
