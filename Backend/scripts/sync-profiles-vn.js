...
async function main() {
  ...
  const zaloToken = await redis('GET', 'phuocthanh_zalo_access_token');
  if (!zaloToken) {
    console.error('❌ Không tìm thấy phuocthanh_zalo_access_token trong Redis');
  ...
  const raw = await redis('GET', 'phuocthanh_oa_followers');
  if (!raw) {
    console.error('❌ Không có dữ liệu followers (key: phuocthanh_oa_followers)');
  ...
  for (let i = 0; i < followers.length; i++) {
    ...
    const cached = await redis('GET', `phuocthanh_profile:${userId}`);
    if (cached) {
      try {
    ...
    try {
      ...
      if (result?.error === 0 && result?.data?.display_name) {
        ...
        await redis('SET', `phuocthanh_profile:${userId}`, value, 'EX', 7776000);
        success++;
