...
async function fetchOutagesForSubOrg(subOrgCode) {
  ...
  for (let page = 1; page <= 10; page++) {
    ...
    // (KHÔNG break theo length===0 + 50 trang: gây ~900 request/sync → rate-limit → bỏ qua đơn vị như Hiệp Đức)
...
async function syncOutages() {
  // Chỉ cào ĐÚNG đơn vị của OA này (Hiệp Đức, phụ trách Phước Thành) — 1 request/lần.
  // Cào cả 18 đơn vị bị EVNCPC rate-limit (429) từ IP VPS → các đơn vị cuối (Hiệp Đức) bị bỏ qua.
...
// subOrgCode mặc định = đơn vị của OA này (Phước Thành). Truyền '' hoặc 'all' để xem toàn TP.
...
.footer { background:#f5f5f5; padding:8px 18px; font-size:11px; color:#888; text-align:right; }
  ...
  <div class="header">
    <div class="logo">⚡</div>
    <div>
      <div class="htitle">Lịch tạm ngừng cấp điện — Phước Thành</div>
...
function formatText(items, query = '') {
  ...
  return `⚡ LỊCH TẠM NGỪNG CẤP ĐIỆN — Phước Thành\n━━━━━━━━━━━━━━━━━━━\n${lines.join('\n\n')}\n━━━━━━━━━━━━━━━━━━━\n📍 Nguồn: EVNCPC`;
}
